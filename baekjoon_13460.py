...
def bfs(ri,rj,bi,bj,d):
    queue=deque([(ri,rj,bi,bj,d)])
    visited.append((ri,rj,bi,bj))
    while queue:
        cri,crj,cbi,cbj,cd=queue.popleft()
        if cd>10: # 10번 이내에 성공하지 못한 경우
            return -1
        for i in range(4):
            nri,nrj,rcnt=move(cri,crj,dx[i],dy[i]) # 빨간 구슬이 벽에 부딪히거나, 구멍에 도달했을 경우
            nbi,nbj,bcnt=move(cbi,cbj,dx[i],dy[i]) # 파란 구슬이 벽에 부딪히거나, 구멍에 도달했을 경우
            if board[nbi][nbj]!='O':
                if board[nri][nrj]=='O':
                    return cd
                # 같은 위치에 있으면 안되기 때문
                if nri==nbi and nrj==nbj:
                    if rcnt>bcnt: # 빨간 구슬이 더 많이 움직였을 경우
                        nri,nrj=nri-dx[i],nrj-dy[i]
                    else:
                        nbi,nbj=nbi-dx[i],nbj-dy[i]

                if (nri,nrj,nbi,nbj) not in visited:
                    visited.append((nri,nrj,nbi,nbj))
                    queue.append((nri,nrj,nbi,nbj,cd+1))
    # 빨간 구슬이 구멍에 도달하지 못하는 경우
    return -1
